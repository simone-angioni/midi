    ...
    with tqdm(total=len(dataloader)) as bar_train:
        for batch_num, batch in enumerate(dataloader):
            ...
            if save_steps % save_checkpoint_steps == 0:
                ...
                # TODO Saving more than just state dict to resume training with more parameters
                state = {
                    'epoch': cur_epoch,
                    'state_dict': model.state_dict(),
                    'optimizer': opt.state_dict()
                }
                filepath = 'gpt2_rpr_checkpoint_' + str(cur_epoch) + '_epoch_' + str(save_steps) + '_steps_' + str(round(float(out), 4)) + '_loss.pth'
